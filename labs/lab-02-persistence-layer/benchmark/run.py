...
def build_latency_histogram(latencies):
    ...
    for low, high in buckets:
        if high is None:
            count = sum(1 for v in latencies if v >= low)
            label = f'{low:>3}ms+'
        else:
            count = sum(1 for v in latencies if low <= v < high)
...
def write_run_summary(run_dir):
    rows = load_timeseries_rows(run_dir / 'timeseries.csv')
    ...
    k6_counts = parse_k6_counts(run_dir / 'k6_summary.json')
    ...
    print(
        'Latency p50/p90/p99: '
        f"{summary['latency_ms']['p50']:.2f} / {summary['latency_ms']['p90']:.2f} / {summary['latency_ms']['p99']:.2f} ms"
    )
    if db_latencies:
        print(
            'DB write p50/p90/p99: '
            f"{summary['db_latency_ms']['p50']:.2f} / {summary['db_latency_ms']['p90']:.2f} / {summary['db_latency_ms']['p99']:.2f} ms"
        )
    print(
        'Throughput avg/peak: '
...
def run_sampler(metrics_url, output_path, stop_event, scrape_interval_seconds, summary_interval_seconds):
    ...
    with output_path.open('w', encoding='utf-8') as f:
        f.write('timestamp_s,vus,latency_ms,db_latency_ms,memory_mb,messages_total,dropped_total,db_errors_total,error_rate_pct,throughput_msgs_s\n')
